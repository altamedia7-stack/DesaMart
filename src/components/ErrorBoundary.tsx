      ...
      try {
        if (this.state.error?.message) {
          ...
          if (parsed.error && parsed.operationType) {
            ...
            errorMessage = `Gagal melakukan operasi ${parsed.operationType} pada ${parsed.path}. Pesan: ${parsed.error}`;
          }
        }
      } catch (e) {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
          <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              {isFirestoreError ? 'Kesalahan Keamanan Database:' : 'Kesalahan Aplikasi:'}
              <br />
              <span className="font-mono text-sm bg-gray-100 p-2 rounded block mt-2 text-left overflow-auto max-h-40">
                {errorMessage}
              </span>
            </p>
            <button
